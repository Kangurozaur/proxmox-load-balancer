...
def perform_migration(migration):
    vmid = migration["vm_id"]
    source_node_name = migration["source_node"]
    target_node_name = migration["target_node"]
    old_score = migration["old_score"]
    new_score = migration["new_score"]
    config = Config.getInstance().config
    migration_logger = logging.getLogger("history")

    if config["test"]:
        migration_logger.info("[TEST] Successfully migrated VM {0} from node {1} to {2}. Score: {3} --> {4} ({5}% decrease)".format(vmid, source_node_name, target_node_name, old_score, new_score, round(((old_score-new_score)/old_score)*100, 2)))
    else:
        try:
            proxmox = ProxmoxAPI(config["connection"]["proxmox"]["url"]["ip"], user=config["connection"]["proxmox"]["auth"]["username"], password=config["connection"]["proxmox"]["auth"]["password"], verify_ssl=False)
            ...
            upid = None
            migration_logger.info("Successfully migrated VM {0} from node {1} to {2}. Score: {3} --> {4} ({5}% decrease)".format(vmid, source_node_name, target_node_name, old_score, new_score, round(((old_score-new_score)/old_score)*100, 2)))
        except:
            migration_logger.warning("Failed to migrate VM {0} from node {1} to {2}".format(vmid, source_node_name, target_node_name))
        return upid
